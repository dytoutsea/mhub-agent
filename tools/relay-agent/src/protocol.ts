...
export function hello(proxyId: string, ticket: string): Hello {
  return {
    type: "HELLO",
    protocol_version: 1,
    proxy_id: proxyId,
...
export function parseRelayMessage(value: string): RelayMessage {
  ...
  if (type === "OPEN_REQUEST") {
    ...
    return {
      ...
      connection_token: text(message, "connection_token", 256),
      host: text(message, "host", 255),
...
export function parseDataAccepted(value: string, connectionId: string) {
  const message = parseObject(value);
...
function text(message: Record<string, unknown>, name: string, maximum: number): string {
...
function requireFields(message: Record<string, unknown>, expected: readonly string[]) {
